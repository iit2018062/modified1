	...
	function fetchData2(nodeData) {
		...
		axios({
			...
			.then((response) => {
				...
				var input = response.data[0].interface;
				//console.log(input);
	...
	function fetchData1() {
		...
		axios({
			...
			.then((response) => {
				...
				for (let node in response.data) {
					if (response.data[node].nodeRole === "leaf") {
						tempLeafs.push(response.data[node]);
					} else if (response.data[node].nodeRole === "spine") {
						tempSpines.push(response.data[node]);
					}
				}
